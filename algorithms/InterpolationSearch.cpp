...
int InterpolationSearch(Employee* arr, int count, int key) {
	...
	// Левая сторона отрезка, должна быть меньше, чем левая.
	// Ключ должен находится в пределах определенных отерзком.
	while (left <= right && key >= arr[left].id && key <= arr[right].id) {
		...
		// Считаем позицию по формуле интерполяции.
		...
		// Ключ находится после текущей позиции.
		if (arr[position].id < key)
			left = position + 1;

		// Ключ находится перед текущей позицией.
		else
			right = position - 1;
	}
